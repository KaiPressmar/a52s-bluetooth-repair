...
public class MainActivity extends Activity {
 ...
 private final int GREEN=Color.rgb(15,122,82), GREEN_DARK=Color.rgb(5,94,61), INK=Color.rgb(18,32,26), MUTED=Color.rgb(102,116,109), BG=Color.rgb(248,250,249), BORDER=Color.rgb(232,238,234);
 ...
 private void buildDashboard(){
  ...
  TextView intro=label("Bluetooth-Telefonie prüfen und gezielt reparieren – klar, sicher und ohne unnötige Eingriffe.",16,Typeface.NORMAL,MUTED);intro.setLineSpacing(dp(2),1f);add(intro,8,-1);
  ...
  add(label("Schnellaktionen",18,Typeface.BOLD,INK),24,-1);
  ...
  add(label("Werkzeuge & Diagnose",18,Typeface.BOLD,INK),24,-1);
 ...
 private LinearLayout action(String symbol,String title,String subtitle,boolean primary,View.OnClickListener click){
  LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(14),dp(13),dp(14),dp(13));row.setBackground(round(primary?GREEN:Color.WHITE,18));row.setElevation(dp(primary?2:1));row.setOnClickListener(click);row.setClickable(true);
 ...
 private LinearLayout card(){LinearLayout v=new LinearLayout(this);v.setGravity(Gravity.CENTER_VERTICAL);v.setPadding(dp(16),dp(15),dp(16),dp(15));v.setBackground(round(Color.WHITE,18));v.setElevation(dp(1));return v;}
